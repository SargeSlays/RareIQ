...
(function(host){
  ...
  function create({borrow,request}){
    ...
    const armedMessage=()=>mode==='ptt'?`Hold Ctrl+Alt+V while speaking · ${pttDown?'host reports key held':'key not held'}${node('studioVoicePractice')?.checked?' · Practice: no actions will run.':'. Accepted commands can control Program and save clips.'}`:node('studioVoicePractice')?.checked?'Listening in Practice · no actions will run.':'Listening · accepted commands can control Program and save clips.';
    function refresh(){
    ...
    async function start(){
      ...
      try{
        ...
        const payload=await request('/api/production/voice/start',{method:'POST',retries:0,body:JSON.stringify({practice:node('studioVoicePractice')?.checked!==false,mode})});
      ...
      }catch(error){if(token===generation){if(created&&!session)session=created;await stop(error?.payload?.reason==='ptt_hotkey_unavailable'?'Ctrl+Alt+V is unavailable; release the conflicting shortcut or choose Wake phrases.':'Voice commands could not start. Check host availability, then try again.');}}
    ...
    refresh();return {start,stop,refresh,status:()=>({state,session_id:session,busy,starting,mode,ptt_down:pttDown})};
  }
