...
function isInteractiveShortcutTarget(target){
  return Boolean(target?.closest?.('button,a[href],summary,label,[role="button"],[role="tab"],[role="menuitem"],[role="checkbox"],[role="switch"]'));
}
...
document.addEventListener("keydown",event=>{
  ...
  const plainShortcut=!event.altKey&&!event.ctrlKey&&!event.metaKey&&!event.shiftKey;
  if(workspace==="soundboard"&&!event.repeat&&!event.altKey&&!event.ctrlKey&&!event.metaKey&&/^[0-9]$/.test(event.key)){
  ...
  }else if(!event.repeat&&!event.altKey&&!event.ctrlKey&&!event.metaKey&&(event.key==="?" || (event.shiftKey&&event.key==="/"))){
    toggleShortcutOverlay();
  }else if(!plainShortcut){
    return;
    ...
    operatorReject();
  }else if(workspace==="live"&&event.key.toLowerCase()==="d"&&!event.repeat){
    operatorDetails();
  }else if(workspace==="live"&&event.key.toLowerCase()==="p"&&!event.repeat){
    openCameraPopout();
  }else if(workspace==="live"&&event.key.toLowerCase()==="z"&&!event.repeat){
    toggleCardZoom();
  }else if(workspace==="live"&&event.key.toLowerCase()==="f"&&!event.repeat){
    openCameraPopout();
  }
});
...
function switchWorkspace(name){
  ...
  if(name==="broadcast") Promise.all([loadProductionSwitcher(),loadProductionScenes(),loadProductionReplay(),loadProductionScreen(),loadOperatorHealth(),loadShowPreflight(),loadProductionSession(),loadRecordingSettings(),loadObsStatus(),loadShowAnalytics(),loadCardShowAnalytics(),loadPackTracker(),loadPackEconomics(),loadBreakHistory()]).catch(error=>notify("Production Tools Unavailable",error.message||String(error),"error"));
  return true;
}
...
function renderCameraFxControls(){const values=cameraFxValues(),filter=cameraFxFilter();[["cameraFxBrightnessValue",`${values.brightness}%`],["cameraFxContrastValue",`${values.contrast}%`],["cameraFxSaturationValue",`${values.saturation}%`],["cameraFxBlurValue",`${values.blur}px`],["cameraFxToleranceValue",`${values.tolerance}%`],["cameraFxSoftnessValue",`${values.softness}%`]].forEach(([id,label])=>{if($(id))$(id).textContent=label});const preview=$("cameraFxPreview"),feed=$("cameraFeed"),canvas=$("cameraFxCanvas");if(preview){preview.src=feed?.src||"";preview.style.filter=preview.dataset.compareClean==="true"?"none":filter}if(feed)feed.style.filter=cameraFxState.enabled&&!values.chroma?filter:"none";if(canvas)canvas.style.filter=cameraFxState.enabled&&values.chroma?filter:"none";const host=$("cameraFxState");if(host){host.dataset.state=cameraFxState.enabled?"live":"off";host.querySelector("span").textContent=cameraFxState.enabled?values.chroma?"Green screen preview active":`${values.preset} preview active`:"Preview bypassed"}if($("cameraFxApply"))$("cameraFxApply").textContent=cameraFxState.enabled?"Bypass Studio Preview":"Apply to Studio Preview";document.body.dataset.cameraFx=cameraFxState.enabled?"on":"off";saveCameraFxPreferences()}
function setCameraFxCompare(active){const preview=$("cameraFxPreview"),button=$("cameraFxCompare");if(preview)preview.dataset.compareClean=active?"true":"false";if(button)button.setAttribute("aria-pressed",String(active));renderCameraFxControls()}
...
function renderShowAnalytics(payload={}){const data=payload.analytics||payload,counts=data.counts||{},events=data.incidents||[],lifecycles=data.incident_lifecycles||[],incidents=lifecycles.length?lifecycles:events,uptime=data.uptime||{},quality=data.session_quality||{},formatSeconds=value=>value===null||value===undefined?"—":Number(value)<60?`${Math.round(Number(value))}s`:sessionTime(Number(value)),formatUptime=metric=>metric?.uptime_percent===null||metric?.uptime_percent===undefined?"N/R":`${Number(metric.uptime_percent).toFixed(2)}%`;if($("analyticsDuration"))$("analyticsDuration").textContent=sessionTime(data.duration_seconds||0);if($("analyticsEvents"))$("analyticsEvents").textContent=String(data.total_events||0);if($("analyticsCueGap"))$("analyticsCueGap").textContent=`${data.average_cue_interval_seconds||0}s`;if($("analyticsIncidents"))$("analyticsIncidents").textContent=String(data.incident_count??incidents.length);if($("analyticsUnresolved"))$("analyticsUnresolved").textContent=String(data.unresolved_incident_count||0);if($("analyticsAckTime"))$("analyticsAckTime").textContent=formatSeconds(data.average_acknowledgment_seconds);if($("analyticsRecoveryTime"))$("analyticsRecoveryTime").textContent=formatSeconds(data.average_recovery_seconds);if($("analyticsProgramUptime"))$("analyticsProgramUptime").textContent=formatUptime(uptime.program_camera);if($("analyticsObsUptime"))$("analyticsObsUptime").textContent=formatUptime(uptime.obs_stream);if($("analyticsRecordingUptime"))$("analyticsRecordingUptime").textContent=formatUptime(uptime.obs_recording);if($("analyticsDestinationUptime"))$("analyticsDestinationUptime").textContent=formatUptime(uptime.destination_route);if($("analyticsReplays"))$("analyticsReplays").textContent=String(counts.replay||0);if($("analyticsGraphics"))$("analyticsGraphics").textContent=String(counts.graphic||0);analyticsRows("analyticsScenes",data.scene_usage,"No scene activity yet.");analyticsRows("analyticsCameras",data.camera_usage,"No camera activity yet.");if($("analyticsIncidentList"))$("analyticsIncidentList").replaceChildren(...(incidents.length?incidents.slice().reverse().map(event=>{const row=document.createElement("div"),copy=document.createElement("div"),title=document.createElement("strong"),detail=document.createElement("small"),status=document.createElement("span"),isLifecycle=Boolean(event.risk_id||event.incident_id);row.className="analytics-incident";row.dataset.state=event.state||"event";title.textContent=event.title||event.risk_id||"Production incident";detail.textContent=event.detail||event.kind||"No detail recorded";status.textContent=isLifecycle?`${event.state==="active"?"UNRESOLVED":"RECOVERED"} · ${formatSeconds(event.duration_seconds)}${event.acknowledgment_seconds===null||event.acknowledgment_seconds===undefined?"":` · ACK ${formatSeconds(event.acknowledgment_seconds)}`}`:new Date(Number(event.timestamp||0)*1000).toLocaleTimeString();copy.append(title,detail);row.append(copy,status);return row;}):[Object.assign(document.createElement("p"),{textContent:"No incidents logged."})]));if($("analyticsRecording"))$("analyticsRecording").textContent=data.recording_verified?"Recording verified":"Recording not verified";if($("analyticsQualitySummary"))$("analyticsQualitySummary").textContent=quality.summary||"Analytics are generated from the current production session timeline.";}
...
function broadcastDestinationCard(destination){
  ...
  const readiness=document.createElement("div"),encoder=document.createElement("span"),next=document.createElement("span");
  ...
  summary.textContent="View setup requirements";
  (destination.setup?.requirements||[]).forEach(value=>{const item=document.createElement("li");item.textContent=value;requirements.append(item);});
  verification.textContent=`Verification: ${destination.setup?.verification_method||"A platform-confirmed connection is required."}`;
  check.type="button";check.className="riq-button broadcast-destination-check";check.textContent="Check status";check.addEventListener("click",()=>loadBroadcastDestinations().then(()=>notify(`${destination.name||"Destination"} Status Checked`,"RareIQ refreshed read-only encoder and connector evidence.","success")).catch(error=>notify("Status Check Failed",error.message||String(error),"error")));
  guide.append(summary,requirements,verification,check);
  card.append(header,capabilities,note,setup,readiness,guide);
  return card;
}
...
let productionRundown=[],productionRundownIndex=0,productionRundownTimer=0,productionRundownRunning=false,productionRundownGeneration=0,productionRundownWaitCancel=null;
...
function confirmRundownReplacement(action){return !productionRundown.length||confirm(`${action} will replace ${productionRundown.length} current cue${productionRundown.length===1?"":"s"}. Continue?`)}
function loadRundownTemplate(){const name=$("rundownTemplateSelect")?.value,template=productionRundownTemplates()[name];if(!template||!confirmRundownReplacement(`Loading “${name}”`))return;productionRundown=structuredClone(template.cues||[]);productionRundownIndex=0;saveProductionRundown();renderProductionRundown();notify("Template Loaded",name,"success");}
...
async function importProductionRundown(file){const payload=JSON.parse(await file.text());if(payload.schema!=="rareiq-rundown-v1"||!Array.isArray(payload.cues))throw new Error("This is not a RareIQ rundown file");const cues=payload.cues.slice(0,100).filter(cue=>cue&&typeof cue.type==="string");if(!confirmRundownReplacement(`Importing ${cues.length} cue${cues.length===1?"":"s"}`))return false;productionRundown=cues;productionRundownIndex=0;saveProductionRundown();renderProductionRundown();notify("Rundown Imported",`${cues.length} cues ready for preflight.`,"success");return true;}
...
function waitProductionRundown(milliseconds){
  return new Promise(resolve=>{
    const finish=()=>{productionRundownTimer=0;productionRundownWaitCancel=null;resolve();};
    productionRundownWaitCancel=finish;
    productionRundownTimer=setTimeout(finish,milliseconds);
  });
}
async function executeProductionCue(cue,{rehearsal=false,isCurrent=()=>true}={}){
  if(!cue||!isCurrent())return;
  if(rehearsal){await waitProductionRundown(Math.min(500,Math.max(100,Number(cue.delay_seconds||0)*1000)));return;}
  if(cue.type==="wait")await waitProductionRundown(Math.max(0,Number(cue.delay_seconds||0)*1000));
  else if(cue.type==="scene"){
    const scene=productionScenes.find(item=>item.id===cue.target);
    if(!scene)throw new Error("Scene is no longer available");
    return await takeProductionScene(scene,{isCurrent});
  }else if(cue.type==="screen"){applyProductionScreenPreset(cue.target);await takeProductionScreen();}
  else if(cue.type==="graphic")await sendProductionGraphic(cue.target==="hide"?"hide":"take");
  else if(cue.type==="replay"){
    const payload=await loadProductionReplay(),latest=payload.highlights?.[0];
    if(!isCurrent())return;
    if(!latest)throw new Error("No replay highlight is ready");
    await takeProductionReplay(latest.id);
  }else if(cue.type==="live"){await hideProductionScreen();if(isCurrent())await stopProductionReplay();}
  else if(cue.type==="sound-stop")stopAllSoundboardAudio();
}
async function goProductionRundown(){
  const cue=productionRundown[productionRundownIndex];
  if(!cue||productionRundownRunning)return;
  productionRundownRunning=true;
  const generation=++productionRundownGeneration,isCurrent=()=>generation===productionRundownGeneration;
  const rehearsal=Boolean($("rundownRehearsal")?.checked);
  try{
    if(cue.type!=="wait"&&Number(cue.delay_seconds)>0)await waitProductionRundown(Number(cue.delay_seconds)*1000);
    if(!isCurrent())return;
    const outcome=await executeProductionCue(cue,{rehearsal,isCurrent});
    // Stop invalidates this run even when its already-sent action finishes later.
    if(!isCurrent())return;
    if(outcome?.partial){
      if($("rundownStatus"))$("rundownStatus").textContent="Paused · scene action needs attention";
      notify("Rundown Paused","The scene changed partially. Resolve its warnings before continuing.","warning");
      return outcome;
    }
    const canAdvance=productionRundownIndex<productionRundown.length-1;
    if(canAdvance)productionRundownIndex++;
    saveProductionRundown();renderProductionRundown();
    notify(rehearsal?"Cue Rehearsed":"Cue Fired",cue.label,"success");
    if(cue.auto_follow&&canAdvance)productionRundownTimer=setTimeout(()=>{if(isCurrent())goProductionRundown().catch(error=>notify("Cue Failed",error.message||String(error),"error"));},50);
  }catch(error){if(isCurrent())throw error;}
  finally{if(isCurrent())productionRundownRunning=false;}
}
function stopProductionRundown(){productionRundownGeneration++;clearTimeout(productionRundownTimer);productionRundownTimer=0;productionRundownWaitCancel?.();productionRundownRunning=false;if($("rundownStatus"))$("rundownStatus").textContent="Automation stopped · selected cue preserved";notify("Rundown Stopped","No further cues will auto-follow.","success");}
function clearProductionRundown(){if(!productionRundown.length)return notify("Rundown Already Empty","There are no cues to clear.","warning");if(!confirmRundownReplacement("Clearing the rundown"))return false;productionRundown=[];productionRundownIndex=0;saveProductionRundown();renderProductionRundown();notify("Rundown Cleared","The cue list is empty.","success");return true;}
...
const productionInputThumbnails=new WeakMap();
function renderProductionInputThumbnail(button,slotState,generation=0){
  const slot=Number(button.dataset.productionSlot),assigned=Boolean(slotState?.source_id),ready=assigned&&Boolean(slotState?.connected);
  let image=button.querySelector("img"),empty=button.querySelector(".production-input-empty");
  if(!image)return;
  if(!empty){
    empty=document.createElement("span");empty.className="production-input-empty";empty.id=`productionInputStatus${slot}`;button.append(empty);
    const described=(button.getAttribute("aria-describedby")||"").split(/\s+/).filter(Boolean);
    if(!described.includes(empty.id))button.setAttribute("aria-describedby",[...described,empty.id].join(" "));
  }
  const key=JSON.stringify([slotState?.source_id||null,ready,generation]),previous=productionInputThumbnails.get(button);
  if(previous?.key===key&&previous.state!=="error")return;
  const entry={key,state:ready?"loading":assigned?"offline":"unassigned"};productionInputThumbnails.set(button,entry);
  const unavailable=(state,message)=>{
    entry.state=state;button.dataset.thumbnailState=state;image.hidden=true;image.removeAttribute("src");
    empty.hidden=false;empty.textContent=message;button.title=`Camera ${slot} · ${message}`;
  };
  if(!ready){unavailable(entry.state,assigned?"Source offline":"No source assigned");return;}
  // A fresh image isolates delayed events from a previously assigned source.
  const replacement=image.cloneNode(false);replacement.removeAttribute("src");replacement.hidden=true;
  image.replaceWith(replacement);image=replacement;image.alt=`Camera ${slot} preview`;
  button.dataset.thumbnailState="loading";empty.hidden=false;empty.textContent="Loading preview…";
  image.onload=()=>{
    if(productionInputThumbnails.get(button)!==entry)return;
    entry.state="ready";button.dataset.thumbnailState="ready";image.hidden=false;empty.hidden=true;
    button.title=`Select Camera ${slot} for Preview`;
  };
  image.onerror=()=>{if(productionInputThumbnails.get(button)===entry)unavailable("error","Preview unavailable · Refresh Sources to retry");};
  image.src=`/api/camera-slots/${slot}/stream?g=${generation}`;
}
function renderProductionSwitcher(state={}){productionSwitcherState={...productionSwitcherState,...state};const program=Number(productionSwitcherState.program_slot)||1,preview=Number(productionSwitcherState.preview_slot)||2,generation=Number(productionSwitcherState.generation)||0,slots=Array.isArray(productionSwitcherState.slots)?productionSwitcherState.slots:[],slotState=id=>slots.find(slot=>Number(slot.slot_id)===Number(id))||null,slotReady=id=>{const slot=slotState(id);return Boolean(slot?.source_id&&slot?.connected)},slotMessage=id=>{const slot=slotState(id);if(!slot?.source_id)return `Camera ${id} · No source assigned`;return slot.connected?`Camera ${id}`:`Camera ${id} · Source offline`},programReady=slotReady(program),previewReady=slotReady(preview);if($("productionProgramLabel"))$("productionProgramLabel").textContent=slotMessage(program);if($("productionPreviewLabel"))$("productionPreviewLabel").textContent=slotMessage(preview);if($("productionProgramPreview"))$("productionProgramPreview").src=`/api/camera-slots/${program}/stream?g=${generation}`;if($("productionPreviewPreview"))$("productionPreviewPreview").src=`/api/camera-slots/${preview}/stream?g=${generation}`;[["productionProgramPreview",program,programReady],["productionPreviewPreview",preview,previewReady]].forEach(([id,slot,ready])=>{const monitor=$(id)?.closest(".production-monitor");if(!monitor)return;monitor.dataset.sourceState=ready?"connected":slotState(slot)?.source_id?"offline":"unassigned";monitor.dataset.sourceMessage=ready?"":slotMessage(slot)});if($("productionTransition"))$("productionTransition").value=productionSwitcherState.transition||"fade";if($("productionDuration"))$("productionDuration").value=String(productionSwitcherState.duration_ms||500);if($("productionCut"))$("productionCut").disabled=!previewReady||preview===program;if($("productionAuto"))$("productionAuto").disabled=!previewReady||preview===program;document.querySelectorAll("[data-production-slot]").forEach(button=>{const slot=Number(button.dataset.productionSlot),ready=slotReady(slot);button.classList.toggle("is-program",slot===program);button.classList.toggle("is-preview",slot===preview);button.dataset.sourceState=ready?"connected":slotState(slot)?.source_id?"offline":"unassigned";button.disabled=slot===program||!ready;button.title=ready?`Select Camera ${slot} for Preview`:slotMessage(slot);renderProductionInputThumbnail(button,slotState(slot),generation);});}
...
async function takeProductionScene(scene,{isCurrent=()=>true}={}){
  if(!isCurrent())return {cancelled:true};
  const state=await api(`/api/production/scenes/${encodeURIComponent(scene.id)}/take`,{method:"POST"}),warnings=[];
  if(!isCurrent())return {...state,cancelled:true};
  renderProductionSwitcher(state);renderProductionScreen(state.screen||{});
  if(state.obs_warning)warnings.push("OBS did not confirm the mapped scene.");
  if(scene.soundboard_action==="stop")stopAllSoundboardAudio();
  if(scene.spotify_action==="play"||scene.spotify_action==="pause"){
    try{await spotifyCommand(scene.spotify_action);}
    catch{warnings.push(`Spotify ${scene.spotify_action} did not complete.`);}
  }
  if(!isCurrent())return {...state,cancelled:true};
  document.querySelectorAll(".production-scene-card").forEach(card=>card.classList.toggle("is-live",card.dataset.sceneId===scene.id));
  const detail=`Camera ${state.program_slot} · ${scene.transition}${warnings.length?` · ${warnings.join(" ")}`:""}`;
  logProductionEvent("scene",`Scene: ${scene.name}`,detail);
  notify(warnings.length?"Scene Changed · Attention":"Scene Changed",warnings.length?`${scene.name} is on local Program. ${warnings.join(" ")}`:`${scene.name} is now on local Program.`,warnings.length?"warning":"success");
  return {...state,partial:warnings.length>0,warnings};
}
...
function fillProductionGraphicFromCard(){const context=window.__rareiqCardContext||{},card=isAuthoritativelyVerified(context.snapshot||{})?context.card:null;if(!card){notify("Verified Card Required","Card graphics stay off air until the current identity is verified against a local reference.","warning");return false}const name=card.english_name||card.card_name||card.name||card.printed_name||"Verified Card",set=card.set_name||card.set||"",number=card.collector_number||card.card_number||card.local_card_id||"",image=card.image_url||card.reference_image_url||card.artwork_url||"";if($("productionGraphicKind"))$("productionGraphicKind").value="card";if($("productionGraphicTitle"))$("productionGraphicTitle").value=name;if($("productionGraphicSubtitle"))$("productionGraphicSubtitle").value=[set,number].filter(Boolean).join(" · ")||"Verified current card";if($("productionGraphicPreviewFrame"))$("productionGraphicPreviewFrame").dataset.imageUrl=image;return true;}
...
function spotifyRuntimeRedirectUri(payload={}){const saved=String(payload.redirect_uri||"").trim();if(payload.configured===true&&saved)return saved;const port=window.location.port||"8765";return `http://127.0.0.1:${port}/api/spotify/callback`}
function setSpotifyAvailability(payload={}){const connected=payload.connected===true;["spotifyShuffle","spotifyPrevious","spotifyPlay","spotifyNext","spotifyRepeat","spotifyDevice","spotifyVolume","spotifySearch","spotifyRefresh","spotifyToolPrevious","spotifyToolPlay","spotifyToolNext"].forEach(id=>{if($(id))$(id).disabled=!connected;});const searchButton=document.querySelector("#spotifySearchForm button[type=submit]");if(searchButton)searchButton.disabled=!connected;if($("spotifySearch"))$("spotifySearch").placeholder=connected?"Tracks, artists, or playlists":payload.configured?"Authorize Spotify to search":"Complete setup to search";document.querySelector(".spotify-shell")?.classList.toggle("spotify-disconnected",!connected)}
...
function creatorShortcutTargetIsEditable(target){return isTypingTarget(target)||isInteractiveShortcutTarget(target);}
function handleCreatorRevealShortcut(event){
  if(document.body.dataset.ui4Workspace!=="creator"||isTypingTarget(event.target)||event.repeat)return;
  if(creatorShortcutTargetIsEditable(event.target)&&event.key!=="Escape")return;
  const armed=$("creatorRevealArming")?.hidden===false;let action=null;
  if(event.code==="Space"&&!event.altKey&&!event.ctrlKey&&!event.metaKey&&!event.shiftKey&&armed)action=()=>$("creatorRevealNow")?.click();
  else if(event.key==="Escape"&&armed)action=()=>$("creatorCancelReveal")?.click();
  else if(event.altKey&&!event.ctrlKey&&!event.metaKey&&event.key==="1")action=()=>previewCreatorAnimation("low");
  else if(event.altKey&&!event.ctrlKey&&!event.metaKey&&event.key==="2")action=()=>previewCreatorAnimation("medium");
  else if(event.altKey&&!event.ctrlKey&&!event.metaKey&&event.key==="3")action=()=>previewCreatorAnimation("grail");
  else if(event.altKey&&!event.ctrlKey&&!event.metaKey&&event.key.toLowerCase()==="n")action=()=>$("creatorNextPack")?.click();
...
const COLLECTION_WORKSPACE_PANELS={
  ...
  boxes:[".inventory-manager",".inventory-manager>.collection-ledger-head",".inventory-manager>.inventory-metrics",".inventory-valuation",".approved-inventory-intake","#inventoryIntakeForm","#inventoryLookupForm",".inventory-stock-browser",".inventory-price-watchlist",".collection-goals",".collection-set-progress",".collection-duplicates"],
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  $("cameraFxCompare")?.addEventListener("keydown",event=>{if((event.key===" "||event.key==="Enter")&&!event.repeat){event.preventDefault();setCameraFxCompare(true)}});
  $("cameraFxCompare")?.addEventListener("keyup",event=>{if(event.key===" "||event.key==="Enter"){event.preventDefault();setCameraFxCompare(false)}});
  ...
  $("rundownClear")?.addEventListener("click",clearProductionRundown);
