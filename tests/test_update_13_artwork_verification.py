...
@pytest.mark.skipif(
    not Path(r"F:\RareIQ\captures\card_1784259767746.jpg").is_file(),
    reason="Latest live Water-symbol regression crop is unavailable.",
)
def test_latest_live_variant_is_recovered_outside_hash_shortlist() -> None:
    from rareiq.services.vision_optimizer_service import VisionOptimizerService
    crop = cv2.imread(r"F:\RareIQ\captures\card_1784259767746.jpg")
    crop = VisionOptimizerService._normalize(crop)
    matches = ArtworkIndexService().search(crop)["matches"]
...
@pytest.mark.skipif(
    not Path(r"F:\RareIQ\captures\card_1784254577179.jpg").is_file(),
    reason="Saved live Horsea regression crop is not available.",
)
def test_saved_horsea_crop_demotes_csv5_false_match() -> None:
    service = ArtworkIndexService()
    crop = cv2.imread(r"F:\RareIQ\captures\card_1784254577179.jpg")

    matches = service.search(crop, limit=10)["matches"]
    positions = {item["id"]: index for index, item in enumerate(matches)}

    horsea = "pokipair-gem_pack_vol_5-87e725b2f54ec31f"
