...
(async()=>{
  ...
  try{
    const page=await browser.newPage({viewport:{width:1920,height:1080}}),requests=[],blocked=[],errors=[];
    ...
    await page.route('**/api/**',async route=>{
      ...
      if(url.pathname.startsWith('/api/production/voice/')){
        ...
        const result=url.pathname.endsWith('/start')?{ok:true,state:'armed',session_id:'qa-offline',practice:true}:url.pathname.endsWith('/stop')?{ok:true,state:'stopped'}:{ok:true,state:'armed',practice:true,last_result:{state:'validated',message:'QA fixture: Practice command validated; no action taken.'}};
    ...
    const report={layer:'Served Edge UI + offline synthetic AudioWorklet + inert host-response fixtures',input:syntheticWav?'provided synthetic WAV':'generated synthetic tone',capturedSyntheticUtterance:syntheticWav?'.tmp/refinish/voice/captured-synthetic-utterance.wav':null,mediaStarts:0,actualVoiceRequests:0,requests,blocked};
    fs.writeFileSync('.tmp/refinish/voice/qa-results.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
